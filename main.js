   ...
   if (pairingCode && !XeonBotInc.authState.creds.registered) {
      ...
      if (!!phoneNumber) {
         ...
         if (!Object.keys(PHONENUMBER_MCC).some(v => phoneNumber.startsWith(v))) {
            console.log(chalk.bgBlack(chalk.redBright("Start with country code of your WhatsApp Number, Example : +237656520674")))
            process.exit(0)
         }
      } else {
         phoneNumber = await question(chalk.bgBlack(chalk.greenBright(`Please type your WhatsApp number 😍\nFor example: +237656520674 : `)))
         phoneNumber = phoneNumber.replace(/[^0-9]/g, '')
...
XeonBotInc.ev.on("connection.update",async  (s) => {
        ...
        if (connection == "open") {
			...
			await delay(1999)
            console.log(chalk.yellow(`\n\n                  ${chalk.bold.blue(`ད𝙆𝙚𝙧𝙢𝘽𝙪𝙜 𝙑2༒`)}\n\n`))
            console.log(chalk.cyan(`< ================================================== >`))
	        console.log(chalk.magenta(`\n${themeemoji} YT CHANNEL: KermHackTools-s9s`))
            ...
            console.log(chalk.magenta(`${themeemoji} CREDIT: KgTech`))
        }
        if (
            connection === "close" &&
            lastDisconnect &&
